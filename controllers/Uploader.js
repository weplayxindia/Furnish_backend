    ...
    try {
        ...
        csvtojson()
            ...
            .then(async (csvData) => {
                for (let i = 0; i < csvData.length; i++) {
                    if (!csvData[i]['Price']) {
                        console.log(csvData[i]['Price'])
                        console.warn(`Price is missing for record at index ${i}. Defaulting to '0'.`);
                    }
                    if (!csvData[i]['AMAZON & FLIPKART PRICE']) {
                        console.warn(`Amazon/Flipkart price is missing for record at index ${i}. Defaulting to '0'.`);
                    }
                    
                    const categoryName = csvData[i]['CATEGORY'];
                    const subcategoryName = csvData[i]['PRODUCT TYPE'];
                    let priceValue = csvData[i]['Price'];
                    let amazon_flipkart_price = csvData[i]['AMAZON & FLIPKART PRICE']

                    priceValue = priceValue.replace(/[^\d.]/g, '').trim();
                    amazon_flipkart_price = amazon_flipkart_price.replace(/[^\d.]/g, '').trim();
                    ...
                    const otherPrice = parseFloat(amazon_flipkart_price);
                    if (isNaN(otherPrice)) {
                        console.error(`Invalid price for record at index ${i}:`, csvData[i]);
                        continue;
                    }
                    ...
                    try {
                        ...
                        for (const file of files) {
                            ...
                            if (fs.existsSync(imagePath)) {
                                const uploadImage = await uploadToCloudinary(imagePath, `Ranbanka-Furniture-${categoryName}-${subcategoryName}`);
                                images.push(uploadImage.secure_url);
                            } else {
                                console.error('File does not exist:', imagePath);
                            }
                    ...
                    if (images.length > 0) {
                        const newProduct = new Product({
                            ...
                            width : csvData[i]['W'],
                            height : csvData[i]['H'],
                            assembly : csvData[i]['ASSEMBLY'],
                            colore : csvData[i]['COLORE'],
                            warrenty: csvData[i]['WARRENTY'],
                            amazon_flipkart_price : otherPrice,
                            ...
                            woodFinish: csvData[i]['PREMRI MATERIAL'].split(',').map(finish => finish.trim()),
                            subcategory: subcategory._id,
                            stock: parseInt(csvData[i]['Stock'], 10) || 0
                        });
